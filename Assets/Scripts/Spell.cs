...
public class Spell
{
    ...
    public readonly CastType m_CastType;
    public readonly int m_Size;

    public float m_SpellDamage;
    public float m_ManaCost;    

    public Spell(int _size, ESpellType _spelltype, CastType _casttype)
    {
    ...
    {
        if (m_CastType is BallCastType)
        {
            m_SpellDamage = m_Size * 10 + 30;
            m_ManaCost = m_Size * 10 + 10;
        }
        else if (m_CastType is LanceCastType)
        {
            m_SpellDamage = m_Size * 10 + 50;
            m_ManaCost = m_Size * 10 + 20;
        }
        else if (m_CastType is GroundCircleCastType)
        {
